...
class OrderControllerTests {
	...
	void testGetTotalAmount() throws Exception {
        ...
        list.add(1);
        list.add(3);
		MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.post(uri).contentType(MediaType.APPLICATION_JSON_VALUE).content(new Gson().toJson(list))).andReturn();
	...
	void testGetdeliveryDetail() throws Exception {
		...
		MvcResult mvcResult = this.mvc.perform(MockMvcRequestBuilders.get(uri).contentType(MediaType.APPLICATION_JSON_VALUE)).andReturn();
		int status = mvcResult.getResponse().getStatus();
		assertEquals(200, status);	
	}
