...
class OrderServiceImplTest {
	...
	void testSaveOrderDetails() {
		when(service.saveOrderDetails(any(User.class))).thenReturn(order);
		assertTrue(order.equals(service.saveOrderDetails(new User())));
	}
	...
	void testGetdeliveryDetail() {
		when(service.getdeliveryDetail(any(Integer.class))).thenReturn(delivery);
		assertTrue(delivery.equals(service.getdeliveryDetail(1)));
	}
